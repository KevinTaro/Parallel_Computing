...
Dynamic sliding window data loader for Whole Slide Images (WSIs).
...
class WSISlidingWindowDataset(Dataset):
    ...
    def _process_patch(coords: Tuple[int, int], wsi_path: str, patch_size: int,
                       ...
                       rejection_ratio: float) -> Tuple[int, int, str]:
        """
        Processes a single patch to determine if it contains tissue.
        This static method is designed to be used by multiprocessing.Pool.
        """
        x, y = coords
        try:
            # OpenSlide object must be created here for multiprocessing compatibility
            with openslide.OpenSlide(wsi_path) as slide:
                ...
                if white_ratio >= rejection_ratio:
                    return (x, y, f"discarded (reason: white_ratio {white_ratio:.2f} >= {rejection_ratio})")
                ...
                if black_ratio >= rejection_ratio:
                    return (x, y, f"discarded (reason: black_ratio {black_ratio:.2f} >= {rejection_ratio})")

                # If not discarded, it's a selected patch
                return (x, y, f"selected (white: {white_ratio:.2f}, black: {black_ratio:.2f})")

        except Exception as e:
            return (x, y, f"discarded (reason: error processing patch: {e})")

    def _create_grid(self) -> List[Tuple[int, int]]:
        """
        Generates a grid of (x, y) coordinates by performing high-precision
        filtering on every potential patch at level 0 using multiprocessing.
        """
        potential_coords = []
        # Adjust grid to not go beyond image dimensions with the patch
        grid_height = self.wsi_height - self.patch_size + 1
        grid_width = self.wsi_width - self.patch_size + 1
        ...
        print(f"[*] 偵測到 {num_workers} 個 CPU 核心。開始使用 {num_workers} 個背景工作程序掃描 {len(potential_coords)} 個候選區塊...")
        ...
        coordinates = []
        # Use a multiprocessing pool to parallelize the patch processing
        with Pool(processes=num_workers) as pool:
            results = pool.map(process_func, potential_coords)

        # Process results and log them
        for x, y, status in results:
            # The batch number is not well-defined here, so we log coordinate by coordinate
